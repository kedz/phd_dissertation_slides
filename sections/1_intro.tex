...
\begin{tikzpicture}
    ...
    \node[draw,align=center,dashed] (gen) at (9,0) {\alert<3>{Generation} \\ \alert<3>{Model}};
        ...
        \node at ($(P2)+(0,-1.0)$) {Focus of Generation Researchers};
...
\item How do learned models of sentence extraction make predictions 
  ...
  \only<3>{
\item  How to design reliable neural natural language generation (NLG)
    models?
    \begin{itemize}
        ...
        respect
        the semantics of the input meaning representation and proivde some surface level control.
  \end{itemize}
}
...
\begin{frame}{Contributions}
    \begin{itemize}
        ...
        \item Alignment Training for controlling realization order.
           % \item We propose an input transformation to make arbitrary seq2seq models controllable at the level of surface order.
        \end{itemize}
    \end{itemize}
\end{frame}
